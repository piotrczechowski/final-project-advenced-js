    ...
    if (i <= rating) {
      ...
      if (lastStar > 0) {
        result += `<span class="last-star-js" data-value="${i}"><svg xmlns="http://www.w3.org/2000/svg" width="16" viewBox="0 0 20 19" fill="none">
                    ...
                    <stop offset="${
                      100 - lastStar
                    }%" stop-color="rgba(244, 244, 244, 0.20)" />
...
export function getExerciseModal(data) {
  return `<div class="modal-info" data-id="${data._id}">
    ...
    <div class="modal-image-vrapper">
      <img loading="lazy" class="modal-img" src="${data.gifUrl}" alt="${
    data.name
  }" />
    </div>
    <div class="modal-content-wrapper">
      <div class="card-wrapper">
        ...
        <div class="rating-modal-container-wrapper"><p>${data.rating.toFixed(
          1
        )}</p>${getRating(data.rating)}</div>
      </div>
